...
from machinate.core import ensure_global_config, ensure_workspace_layout, find_workspace_root, load_json
from machinate.ui import can_prompt_interactively, prompt_text


def register(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    workspace_parser = subparsers.add_parser("workspace", help="Manage Machinate workspaces")
    workspace_subparsers = workspace_parser.add_subparsers(dest="workspace_command", required=True)

    workspace_init = workspace_subparsers.add_parser("init", help="Create a Machinate workspace scaffold")
...
def cmd_workspace_show(args: argparse.Namespace) -> int:
    ...
    if root is None:
        raise SystemExit("no Machinate workspace detected")

    manifest_path = root / ".machinate" / "workspace.json"
    if not manifest_path.exists():
        raise SystemExit(f"workspace manifest missing: {manifest_path}")

    manifest = load_json(manifest_path)
    print("Machinate workspace")
