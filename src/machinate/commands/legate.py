...
def build_data_report_prompt(
    *,
    ...
    return f"""You are acting as a Machinate delegated reporting agent running through Codex CLI in non-interactive exec mode.

Objective:
- Inspect the dataset at `{dataset_path}`.
- Use local shell and Python commands as needed to understand the dataset.
- Produce a structured JSON data report for Machinate.
...
def cmd_report(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="machinate-legate-") as tempdir:
