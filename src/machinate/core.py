...
PIPELINE_SENTINEL = Path("machinate.toml")
...
def require_pipeline_root(explicit_root: str | None = None) -> Path:
    ...
    if detected is None:
        raise SystemExit("no Machinate pipeline found from the current directory; run inside a pipeline repo or pass --pipeline-path")
    return detected
...
def js_fetch_helper_path() -> Path:
    return Path(str(importlib.resources.files("machinate").joinpath("js/fetch_url.mjs")))
