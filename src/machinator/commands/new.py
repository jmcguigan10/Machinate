...
from machinator.ui import MenuChoice, can_prompt_interactively, prompt_multiselect, prompt_select, prompt_text
...
def register(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    new_parser = subparsers.add_parser("new", help="Create new Machinator-managed objects")
    new_subparsers = new_parser.add_subparsers(dest="new_command", required=True)

    new_pipeline = new_subparsers.add_parser("pipeline", help="Create a manual pipeline scaffold")
...
1. If you have not analyzed data yet, prefer the dataset-first flow: `macht grab data`, `macht legate report --data`, then `macht collate pipeline --create`.
2. If you are intentionally working manual-first, run `macht collate pipeline` later to materialize `dataset_facts.toml`, `model.toml`, and `training.toml`.
...
def create_pipeline_scaffold(
    *,
    ...
    if resolved_repo_path.exists() and any(resolved_repo_path.iterdir()):
        raise SystemExit(f"pipeline path already exists and is not empty: {resolved_repo_path}")
    ...
    (resolved_repo_path / "README.md").write_text(pipeline_readme(pipeline_name, pipeline_type, template))
    (resolved_repo_path / ".gitignore").write_text("__pycache__/\n*.py[cod]\n.venv/\noutputs/\n")
    ...
    write_json(
        manifest_path,
        {
            ...
            "supported_tasks": resolved_tasks,
        },
    )
    return {
        "pipeline_name": pipeline_name,
...
def cmd_new_pipeline(args: argparse.Namespace) -> int:
    ...
    print(f"repo path: {scaffold['repo_path']}")
    print(f"registry manifest: {scaffold['manifest_path']}")
    return 0
