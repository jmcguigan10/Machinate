use machinator_ir::ArchitectureSpec;
use serde_json::json;
use std::env;
use std::path::PathBuf;

fn usage() -> ! {
    eprintln!("usage: machinator-ir <validate|diff|migration-plan> <args>");
    std::process::exit(2);
}
            ...
            match (load_spec(&old_path), load_spec(&new_path)) {
                (Ok(old_spec), Ok(new_spec)) => match (old_spec.validate(), new_spec.validate()) {
                    (Ok(()), Ok(())) => emit(json!({
                        ...
                        "old_spec": old_path,
                        "new_spec": new_path,
                        "migration_plan": machinator_ir::build_migration_plan(&old_spec, &new_spec),
                    })),
                    (Err(error), _) | (_, Err(error)) => emit(json!({
                        "ok": false,
                        "error": error.to_string(),
